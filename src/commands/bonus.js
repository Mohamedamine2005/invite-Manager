...
module.exports = {
    ...
    run: (client, message, args) => {
            ...
            if (!args[0]) {
                ...
                var user = message.mentions.users.first() || client.users.cache.get(args[1]) || client.users.cache.find(c => c.id === args[1]) || client.users.cache.find(c => c.username === args[1]) || message.author;
                var points = args[2]
                if (!points) return message.channel.send(
                    new MessageEmbed().setAuthor(message.author.tag, message.author.avatarURL({ dyanmic: true }))
                    .setDescription(
